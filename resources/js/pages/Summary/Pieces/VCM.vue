<template>
  <PlantOutputChart
    title="Output by QA VCM"
    eyebrow="QA VCM Monthly Output"
    plant="vcm"
    :raw-data="rawData"
    :targets="targets"
  ...
  setup() {
    ...
    const fetchData = async () => {
        ...
        if (monthsData[currentMonth]) {
            rawData.value = { 'QA VCM': monthsData[currentMonth] }
        }
    }

    const reloadTargets = async () => {
        const year    = new Date().getFullYear()
        invalidateTargets('vcm', year)
        targets.value = await fetchTargets('vcm', year)
    }
